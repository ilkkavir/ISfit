## file:readFaradayDir.R

##
## Read ACFs and Faraday rotation angles from result files of ellipseFit or faradayFit
##
## Arguments:
##  dpath     data path
...
##              beta   in case of ellipseFit results, the estimated apparent scattering angle
##                     in case of faradayFit results, the scattering angle calculated from measurement geometry
##              phi    Rotation angles
...
readFaradayDir <- function(dpath,recursive=FALSE,lags='all',stdThresh=NULL)
  {
    ...
    beta       <- array(0,dim=c(nRange,nFile))
    varbeta    <- array(0,dim=c(nRange,nFile))
    phi        <- array(0,dim=c(nRange,nFile))
    varphi     <- array(0,dim=c(nRange,nFile))
    ...
    for (k in seq(nFile)){
      ...
      var[,,k]         <- dTmp$var[,lags]
      beta[,k]         <- dTmp[["beta"]]
      if(!is.null(dTmp[["varbeta"]])) varbeta[,k]        <- dTmp[["varbeta"]]
      phi[,k]          <- dTmp[["phi"]]
      varphi[,k]       <- dTmp[["varphi"]]
    ...
    reslist[["var"]]        <- var
    reslist[["beta"]]       <- beta
    reslist[["varbeta"]]    <- varbeta
    reslist[["phi"]]        <- phi
    reslist[["varphi"]]     <- varphi
