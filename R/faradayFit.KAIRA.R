faradayFit.KAIRA  <- function( xxpath , yypath , xypath , yxpath , opath, heightLimits.km , beginTime=c(2012,1,1,0,0,0),endTime=c(2100,1,1,0,0,0) , timeRes.s , maxdev=1, azXpol=318 , azelBore=c(0,90) , handedness=-1 )
    {
        #
        ...
        for( k in seq( nIper ) ){
            ...
            if( nFiles > 0 ){
                ...
                latitude <- longitude <- height <- ran.gate <- phi <- varphi <- ecc <- varecc <- rep(NA,nh)
                ...
                resFile <- file.path( opath , paste( sprintf( '%13.0f' , trunc( iperLimits[k+1]  * 1000 ) ) , "FR.Rdata" , sep=''))
