          ...
          runcmd <- expression(
              {
                  ...
                  if( nFiles > 0 ){
                      ...
                      # this will need to be replaced with coviriance matrices at some point!
...
# tähän pitää kehittää jonkinlaisia listoja / vektoreita / matriiseja mittauksia, prioreja, yms. varten!
#                                      acf
#                                      var
#                                      lags
#                                      nData
...
# do not stop even when no data is available, we will need the prior etc. for the full-profile analysis
#                          if(any(!nainds)){
                          ...
                          acflist[[h]] <- unlist(acf.site)
                          varlist[[h]] <- unlist(var.site)
                          laglist[[h]] <- unlist(lag.site)
                          nData[h] <- sum(nlags.site)


                          ## aaaaargh!!! liian vaikeaa, kaikesta mahdollisesta pitää tehdä datavektorin mittaisia vektoreita että tähän saadaan jotain järkeä!!!

# tässä kohtaa fitin tilalla tämäm korkeuden lisääminen mittaus/priori listoihin. Tai mitä tähän nyt kehitetäänkään..
...
# tähän sitten itse kokoprofiilifitti. Tuloksesta pitäisi pystyä kaivamaan kaikki allaoleva identtisessä muodossa. Lisäksi lienee syytä tallentaa ainakin kovarianssimatriisi jos vaan mahdollista.
# Priorin pitää olla sen verran tiukka että fitti toimii vaikka gateja puuttuisi välistä tms.!!!
