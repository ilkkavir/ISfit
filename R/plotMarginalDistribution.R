...
plotMarginalDistribution <- function(data,pdf=NULL,jpg=NULL,figNum=NULL,width=8.27,height=2.9225,paper='a4',par=list(Ne=c(10,12),TeR1=c(0,4000),TiR1=c(0,3000),ViR1=c(-400,400)),hLim=NULL,tLim=NULL,bg='white',fg='black',res=300,cex=1.0,allPoints=F,confLimits=c(.682,.954,.996)){
    UseMethod("plotMarginalDistribution")
}

plotMarginalDistribution.character <- function(data,pdf=NULL,jpg=NULL,figNum=NULL,width=8.27,height=2.9225,paper='a4',par=list(Ne=c(10,12),TeR1=c(0,4000),TiR1=c(0,3000),ViR1=c(-400,400)),hLim=NULL,tLim=NULL,bg='white',fg='black',res=300,cex=1.0,allPoints=F,confLimits=c(.682,.954,.996)){

    if(!file.exists(data)) stop(paste("Path",data,"does not exist"))
    if(file.info(data)$isdir) stop("Only contents of individual files can be plotted")
    data <- readPP.MCMC(data,MCMClist=TRUE)
    ...
    do.call( plotMarginalDistribution , args )


}

plotMarginalDistribution.list <- function(data,pdf=NULL,jpg=NULL,figNum=NULL,width=8.27,height=2.9225,paper='a4',par=list(Ne=c(10,12),TeR1=c(0,4000),TiR1=c(0,3000),ViR1=c(-400,400)),hLim=NULL,tLim=NULL,bg='white',fg='black',res=300,cex=1.0,allPoints=F,confLimits=c(.682,.954,.996)){
#
...
#
    ...
    rInds <- rep(T,length(data$height))
    ...
    for(p in seq(length(par))){
        ...
        nmcmc <- data$functionCall$MCMCsettings$niter - data$functionCall$MCMCsettings$burninlength
        d <- matrix(NA,nrow=nheights,ncol=nmcmc)
        for(rii in seq(nheights)){
            if(hInds[[rii]]<=length(data[["MCMClist"]][[1]])){
                if(!is.null(data[["MCMClist"]][[1]][[hInds[rii]]])){
                    d[ rii , ] <- data[["MCMClist"]][[1]][[hInds[rii]]][ , names(par)[p]]
                }
            }
        }

        addDistPlot( d=d , log=xlog , h=data$height[hInds] , xlim=par[[p]] , ylim=hLim2 , xlab=xlab[[1]] , points=allPoints , confLimits=confLimits )
