ISapriori.default.3D <- function( aprioriParam , nIon , ... ){
#
...
#
  ...
  aprioriStd[11]               <- 10                  # perpendicular temperature of first ion species
  ...
  if(nIon>1){
    aprioriStd[16:(nIon*8+7)]  <- rep(
                                         c(
                                           ...
                                           10 ,        # perpendicular ion temperature
  ...
  if(nPar>(nIon+1)*8) aprioriStd[((nIon+1)*8+1):length(aprioriParam)] <- 1e-3 # allow scaling for other sites
...
#  curRow                         <- curRow + 1
  ...
  # electrons and ions usually have the same velocity, but allow differences as well
