ISfit.3D.plasmaline <- function( ddirs='.',pldirs='.' , odir='.' ,  heightLimits.km=NA , timeRes.s=60 , beginTime=c(1970,1,1,0,0,0) , endTime=c(2100,1,1,0,0,0) , fitFun=leastSquare.lvmrq , absLimit=5 , diffLimit=1e-2 , maxLambda=1e30 , maxIter=10 , plotTest=FALSE , plotFit=FALSE , absCalib=FALSE , TiIsotropic=TRUE , TeIsotropic=TRUE , recursive=TRUE , aprioriFunction=ISaprioriH , scaleFun=acfscales , siteScales=NULL, calScale=1, MCMCsettings=list( niter=10000 , updatecov=100 , burninlength=5000 , outputlength=5000 ) , maxdev=2 , trueHessian=FALSE , nCores=1 , plotSpectra=FALSE, ... )
{
                                        #
                                        # we now fit only Vi, must still add Ve and the proper prior model to force Vi_perp = Ve_perp
                                        # the actual plasma line part is stil missing from ISdirectTheoryPL
                                        #
      # 3D incoherent scatter plasma parameter fit using LPI output files in ddirs
      #
      # Multistatic combined ion-plasma line fit in F region. Perpendicular ion and electron velocities are assumed to be equal,
      # but different field-aligned components are allowed.  The plasma line data are assumed to be frequencies of up- and downshifted
      # plasma lines and their standard deviations. 
      #
      #
      #
      # INPUT:
      #   ddirs           a vector of data directory paths, each directory must contain data from exactly one site
      #   pldirs          a vector of plams line data directory paths, each directory must contain data from exactly one site. DATA FORMAT TO BE DECIDED!
      ...
      plfiles <- list()
      pldirs <- unique(pldirs)
      npl <- length(pldirs)
      for( n in seq(npl) ){
          plfiles[[n]] <- dir( pldirs[n] , full.names=T , recursive=recursive , pattern="[[:digit:]]*.Rdata")
      }

      # stop if there is no data
      if ( sum( sapply( dfiles , length ) ) == 0 ) stop( "Could not find any data files." )

      # read timestamps from ion line file names. This is the unix time at end of integration period
      ...
      # timestamps from plasma line data
      tstampsPl <- lapply(plfiles,function(x){sapply(x,function(x){as.numeric(substr(rev(unlist(strsplit(x,.Platform[["file.sep"]])))[1],1,13))/1000})})
          ...
          runcmd <- expression(
              {
                  ...
                  iperPlFiles <- lapply( tstampsPl , function(x,l1,l2){ which( ( x > l1 ) & ( x <= l2 ) ) } , l1=iperLimits[k] , l2=iperLimits[k+1] )

                  
                  # load all ion line data and collect it in a list
                  ...
                  # load all plasma line data and collect it in a list
                  nPlFiles <- sum( sapply( iperPlFiles , length ) )
                  if( nPlFiles > 0 ){

                      pllist <- vector( mode='list', length=npl)

                      for( n in seq( npl ) ){
                          if( length(iperPlFiles[[n]]) > 0 ){
                              pllist[[n]] <- readPlasmaLines( plfiles[[n]][ iperPlFiles[[n]] ] )
                              # some sanity checks
                              if( (pllist[[n]][['azelT']][2]<0) | (pllist[[n]][['azelR']][2]<0) | (pllist[[n]][['radarFreq']]<0) ){
                                  pllist[[n]] <- list()
                                  pllist[[n]][["fup"]] <- matrix(NA)
                                  pllist[[n]][["fdown"]] <- matrix(NA)
                                  pllist[[n]][["varup"]] <- matrix(NA)
                                  pllist[[n]][["vardown"]] <- matrix(NA)
                                  pllist[[n]][["range.km"]] <- NA
                                  pllist[[n]][["azelT"]] <- c(NA,NA)
                                  pllist[[n]][["azelR"]] <- c(NA,NA)
                                  pllist[[n]][["llhT"]] <- c(NA,NA,NA)
                                  pllist[[n]][["llhR"]] <- c(NA,NA,NA)
                                  pllist[[n]][["radarFreq"]] <- NA
                              }
                          }else{
                              pllist[[n]] <- list()
                              pllist[[n]][["fup"]] <- matrix(NA)
                      ...
                      # read up- and downshifted plasma lines etc. will probably need to be a bit different from the ACF reading below..
                                        # read acf, variance, lag, range, pointing directions, and TX / RX location of each data point
                      plfup <- unlist(lapply( pllist , function(x){ return( x[['fup']] )}))
                      plfdown <- unlist(lapply( pllist , function(x){ return( x[['fdown']] )}))
                      plvarup <- unlist(lapply( pllist , function(x){ return( x[['varup']] )}))
                      plvardown <- unlist(lapply( pllist , function(x){ return( x[['vardown']] )}))
                      plran <- 1000*unlist(lapply( pllist , function(x){ return( x[['range.km']] )}))
                      plsinds <- unlist( lapply( seq(npl) , function( i , x ){ return( rep( i , x[[i]][["nGates"]] ) ) } , x=pllist ) )
                      ...
                      plsites <- matrix( nrow=npl , ncol=12 )
                      for(n in seq(npl)){
                          plsites[n,1]     <- n
                          plsites[n,2]     <- pllist[[n]][["radarFreq"]]
                          plsites[n,3:5]   <- pllist[[n]][["llhT"]]
                          plsites[n,6:7]   <- pllist[[n]][["azelT"]]
                          plsites[n,8:10]  <- pllist[[n]][["llhR"]]
                          plsites[n,11:12] <- pllist[[n]][["azelR"]]
                      }
                      ...
                      # if there is data from only one site this is trivial
                      ...
                      # call scaleFun also with the plasma line data to make things easier...
                      pldscales <- scaleFun( plsites )
                      ...
                      # IRI does not work below 70 km
                      hlims <- hlims[hlims>=70000]


                      nh <- length( hlims ) - 1

                      covar <- intersect  <- plintersect <- list()
                      ...
                      plllh <- matrix(nrow=length(plran),ncol=3)
                      for( dind in seq(length(plran))){
                          if(!is.na(plran[dind])) plllh[dind,] <- range2llh( r=plran[dind] , llhT=plsites[plsinds[dind],3:5] , llhR=plsites[plsinds[dind],8:10] , azelT=plsites[plsinds[dind],6:7])
                      }
                      

                      # a list for site indices contributing at each height
                      plcontribsites <- contribSites <- apriori <- list()
                      ...
                      for( h in seq( nh ) ){
                          ...
                          plgateinds     <- which( ( plllh[,3] >= hlims[h] ) & (plllh[,3] < hlims[h+1]) )
                          ...
                          plfup.gate     <- plfup[plgateinds]
                          plfdown.gate   <- plfdown[plgateinds]
                          plvarup.gate   <- plvarup[plgateinds]
                          plvardown.gate <- plvardown[plgateinds]
                          plran.gate     <- plran[ plgateinds ]
                          plllh.gate     <- plllh[ plgateinds ]
                          plsites.gate   <- plsites[ plsinds[plgateinds]  , ] 

                          if(!is.matrix(llh.gate)) llh.gate <- matrix(llh.gate,nrow=1)
                          if(!is.matrix(sites.gate)) sites.gate <- matrix(sites.gate,nrow=1)

                          if(!is.matrix(plllh.gate)) plllh.gate <- matrix(plllh.gate,nrow=1)
                          if(!is.matrix(plsites.gate)) plsites.gate <- matrix(plsites.gate,nrow=1)
                          
                          # remove NA values
                          nainds         <- is.na(acf.gate) | is.na(var.gate)
                          plnainds       <- is.na(plfup.gate) | is.na(plfdown.gate) | is.na(plvarup.gate) | is.na(plfdown.gate)
                          ...
                          if(any(!nainds)){
                              ...
                              plfup.gate     <- plfup.gate[ !plnainds ]
                              plfdown.gate   <- plfdown.gate[ !plnainds ]
                              plvarup.gate   <- plvarup.gate[ !plnainds ]
                              plvardown.gate <- plvardown.gate[ !plnainds ]
                              plllg.gate     <- plllh.gate[ !plnainds ]
                              plsites.gate   <- plsites.gate[ !plnainds ]
                              ...
                              llhTarget      <- colMeans( rbind( llh.gate , plllh.gate ) )
                             ...
                             # beam intersections
                              intersect[[h]] <- list()
                              plintersect[[h]] <- list
                              gainR <- plgainR <- aSite  <- plaSite <- nlags.site <-  rep(NA,nd)
                              kSite <- plkSite <- list()
                              lag.site <- list()
                              acf.site <- list()
                              var.site <- list()
                              ind.site <- list()
                              plfup.site <- plfdown.site <- plvarup.site <- plvardown.site <- list()
                              ...
                              for( s in seq(npl) ){
                                  ...
                                  if(any(is.na(plsites[s,]))){
                                      plaSite[s] <- NA
                                      plkSite[[s]] <- c(NA,NA,NA)
                                  }else{
                                      # the beam widths and antenna types are stored in dscales
                                      plintersect[[h]][[s]] <- beamIntersection( llhT=plsites[s,3:5] , llhR=plsites[s,8:10] , azelT=plsites[s,6:7] , azelR=plsites[s,11:12] , fwhmT=pldscales[s,1] , fwhmR=pldscales[s,3] , phArrT=pldscales[s,2]>0 , phArrR=pldscales[s,4]>0 , freq.Hz=plsites[s,2] )

                                      # conversion from lat, lon, height to range in this gate
                                      plrs1 <- height2range( llhT=plsites[s,3:5] , azelT=plsites[s,6:7] , llhR=plsites[s,8:10] , h=hlims[h] )
                                      plrs2 <- height2range( llhT=plsites[s,3:5] , azelT=plsites[s,6:7] , llhR=plsites[s,8:10] , h=hlims[h+1] )

                                      # gain integral (needed just to check that the pointing directions are ok)
                                      plgainR[s] <- gategain( intersect[[h]][[s]] , c(rs1,rs2) , maxdev=maxdev)

                                      # scattering angle
                                      plaSite[s] <- plintersect[[h]][[s]][["phi"]]

                                      # scattering wave vector
                                      plkSite[[s]] <- plintersect[[h]][[s]][["k.ENU"]]
                                  }
                                  if(is.na(plgainR[s])){
                                      plfup.site[[s]] <- c()
                                      plfdown.site[[s]] <- 0
                                      ...
                                      # data points from this site
                                      pldata.site <- which(plsites.gate[,1]==s)

                                      # average data points from each lag value
                                      plfup.site[[s]] <- sum( plfup.gate[ pldata.site ] / plvarup.gate[ pldata.site ] )
                                      plvarup.site[[s]] <- 1 / sum( 1 / plvarup.gate[ pldata.site ] )
                                      plfup.site[[s]] <- plfup.site[[s]] * plvarup.site[[s]]
                                      plfdown.site[[s]] <- sum( plfdown.gate[ pldata.site ] / plvardown.gate[ pldata.site ] )
                                      plvardown.site[[s]] <- 1 / sum( 1 / plvardown.gate[ pldata.site ] )
                                      plfdown.site[[s]] <- plfdown.site[[s]] * plvardown.site[[s]]
                                      plind.site[[s]] <- s
                                  }
                              ...
                              if(sum(nlags.site)>0){
                                  ...
                                  # the densities in outfmsis are in cm^-3 ... no they are not, I convert them in iri!!!
                                  ...
                                  # do not start from too close to zero
                                  ...
                                  # apriori information
                                  apriori[[h]]   <- aprioriFunction( initParam , nIon=3 , absCalib=absCalib , TiIsotropic=TiIsotropic , TeIsotropic=TeIsotropic , refSite=refsite , siteScales=sScales[,c((h+12),(h+12+nh))] , h=height[h] , B=B[h,] , ...)
                                  ...
                                  fitpar   <- ISparamfitPL(
                                      ...
                                      plfup           = unlist(plfup.site),
                                      plvarup         = unlist(plvarup.site),
                                      plfdown         = unlist(plfdown.site),
                                      plvardown       = unlist(plvardown.site),
                                      plfSite         = plsites[,2],
                                      plaSite         = plaSite,
                                      plkSite         = plkSite,
                                      pliSite         = unlist(plind.site),
                                      B               = B[h,],
                                      initParam       = initParam,
